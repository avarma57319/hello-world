...
# We are assuming the hours are based on the first time-hours entry starting from Sunday, Monday, Tuesday and so on... so choosing variable index
#- for the third time entry which will be Tuesday
week1_sleep[3]


# Create a vector called `week1_sleep_weekdays`
# Assign the weekday values using indice slicing
# Here we will choose indexes for weekdays only starting from position 2=Monday to 6=Friday
...
# Here we will choose variable index starting from position 2 to 6 to choose only weekdays, index 1 and 7 are weekends
weekdays <- days[2:6]
...
matrix(c(student01, student02, student03),nrow=3, ncol=2) -> students_combined
...
factor_recommendations_vector <- factor(recommendations_vector,ordered = TRUE,levels = c("neutral", "no", "yes"))
